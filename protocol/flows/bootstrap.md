...
docs/protocol/AUTHORITY_DIGEST

docs/protocol/protocol/<registered authority paths>
```

For SAPDP v4 and later, Bootstrap copies `SAPDP.md` and every Authority Registry path from the same resolved Git ref, preserving registered paths under `docs/protocol/`. It validates the copied Authority Digest before Runtime entry.
...
- repository and project identity are explicit;
- required directories and artifacts exist;
- generated artifacts conform to their templates;
- protocol version and source ref are resolved and locked;
- Runtime State stops before Product Development Problem execution;
- required handoff information is complete;
- required Git evidence is verifiable.

Recoverable defects return `PATCH REQUIRED`. Missing repository reality, invalid protocol source, destructive overwrite, or unverifiable success returns `FAIL`.

Bootstrap completion is commit-gated. A local-only artifact set does not establish remote completion when remote evidence is required.
